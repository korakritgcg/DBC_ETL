"""
Posted_sales_credit_memos.py — Full load (TRUNCATE once + batched APPEND).
OData page / SQL table : raw.Posted_sales_credit_memos
PK                     : No
"""
...
logger = get_logger("Posted_sales_credit_memos_etl")

ODATA_PAGE = "Posted_sales_credit_memos"
TABLE_NAME = "Posted_sales_credit_memos"
SCHEMA = "raw"
PRIMARY_KEYS = ["No"]
...
def run_etl():
    logger.info("===== START ETL: Posted_sales_credit_memos (full load / batched) =====")
    ...
    if not table_truncated:
        ...
        logger.info("===== END ETL: Posted_sales_credit_memos =====")
        return
