"""
Posted_Sales_Invoice.py — Full Refresh (staged batches → atomic swap).
OData page / SQL table : raw.Posted_Sales_Invoice
PK                     : No
Strategy:
...
logger = get_logger("Posted_Sales_Invoice_etl")

ODATA_PAGE   = "Posted_Sales_Invoice"
TABLE_NAME   = "Posted_Sales_Invoice"
SCHEMA       = "raw"
PRIMARY_KEYS = ['No']
...
def run_etl():
    logger.info("===== START ETL: Posted_Sales_Invoice (Full Refresh / staged swap) =====")
    ...
    odata_url = (
        ...
        f"/ODataV4/Company('{company}')/Posted_Sales_Invoice"
    )
    ...
    try:
        ...
        if total_fetched == 0:
            ...
            logger.info("===== END ETL: Posted_Sales_Invoice =====")
            return
