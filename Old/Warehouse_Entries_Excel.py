"""
Warehouse_Entries_Excel.py — Incremental load using Entry_No as watermark.
PK                     : Entry_No + Line_No
"""
...
logger = get_logger("Warehouse_Entries_Excel_etl")

ODATA_PAGE = "Warehouse_Entries_Excel"
TABLE_NAME = "Warehouse_Entries_Excel"
SCHEMA = "raw"
PRIMARY_KEYS = ["Entry_No", "Line_No"]
...
def run_etl():
    logger.info("===== START ETL: Warehouse_Entries_Excel (incremental) =====")
    ...
    if max_entry_no is None:
        logger.info(
            "Table does not exist or is empty — skipping incremental load. "
            "Run init_load_first_row.py first to create the table."
        )
        logger.info("===== END ETL: Warehouse_Entries_Excel =====")
        return
    ...
    while odata_url:
        ...
        logger.info(
            ...
            f"(last {WATERMARK_COL}: {last_entry_no})"
        )
    ...
    if total_fetched == 0:
        logger.info("No new records since last run.")
        logger.info("===== END ETL: Warehouse_Entries_Excel =====")
        return
    ...
    logger.info(f"Last {WATERMARK_COL} fetched: {last_entry_no}")
    logger.info("===== END ETL: Warehouse_Entries_Excel =====")