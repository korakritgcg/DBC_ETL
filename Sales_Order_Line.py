"""
Sales_Order_Line.py — Full load (TRUNCATE once + batched APPEND).
OData page / SQL table : raw.Sales_Order_Line
...
logger = get_logger("Sales_Order_Line_etl")

ODATA_PAGE = "Sales_Order_Line"
...
def run_etl():
    logger.info("===== START ETL: Sales_Order_Line (full load / batched) =====")
    ...
    odata_url = (
        ...
        f"/ODataV4/Company('{company}')/{ODATA_PAGE}?$top=5000"
    )
    ...
    if not table_truncated:
        ...
        logger.info("===== END ETL: Sales_Order_Line =====")
        return
