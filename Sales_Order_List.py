"""
Sales_Order_List.py — Full Refresh (staged batches → atomic swap).
OData page / SQL table : raw.Sales_Order_List
PK                     : Document_Type, No
Strategy:
...
logger = get_logger("Sales_Order_List_etl")

ODATA_PAGE   = "Sales_Order_List"
TABLE_NAME   = "Sales_Order_List"
SCHEMA       = "raw"
...
def run_etl():
    logger.info("===== START ETL: Sales_Order_List (Full Refresh / staged swap) =====")
    ...
    odata_url = (
        ...
        f"/ODataV4/Company('{company}')/Sales_Order_List"
    )
    ...
    try:
        ...
        if total_fetched == 0:
            ...
            logger.info("===== END ETL: Sales_Order_List =====")
            return
