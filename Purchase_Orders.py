"""
Purchase_Orders.py — Full Refresh (staged batches → atomic swap).
OData page / SQL table : raw.Purchase_Orders
PK                     : Document_Type, No
Strategy:
...
logger = get_logger("Purchase_Orders_etl")

ODATA_PAGE   = "Purchase_Orders"
...
def run_etl():
    logger.info("===== START ETL: Purchase_Orders (Full Refresh / staged swap) =====")
    ...
    try:
        ...
        if total_fetched == 0:
            ...
            logger.info("===== END ETL: Purchase_Orders =====")
            return
